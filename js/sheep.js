...
const SheepManager = {
    ...
    // إعداد النموذج
    setupForm() {
        console.log('📋 إعداد نموذج الأغنام...');
    ...
    async loadSheepData() {
        try {
            console.log('🐑 جاري تحميل بيانات الأغنام...');
            this.sheepData = await API.sheep.getAll();
            console.log(`✅ تم تحميل ${this.sheepData.length} خروف`);
            this.displaySheepTable(this.sheepData);
        } catch (error) {
            console.error('❌ خطأ في تحميل بيانات الأغنام:', error);
            UI.showAlert('خطأ في تحميل بيانات الأغنام: ' + error.message, 'error');
        }
    },
    ...
    async populatePenOptions() {
        try {
            console.log('🏠 جاري تحميل قائمة الحظائر...');
            ...
            console.log('✅ تم جلب الحظائر:', pens);
            
            const penSelect = document.getElementById('pen');
            
            if (!penSelect) {
                console.error('❌ عنصر اختيار الحظيرة غير موجود');
                return;
            }
            ...
            if (!pens || pens.length === 0) {
                penSelect.innerHTML += '<option value="" disabled>لا توجد حظائر - يجب إضافة حظيرة أولاً</option>';
                console.log('⚠️ لا توجد حظائر في النظام');
                
                // عرض رسالة توجيهية
                UI.showAlert('لا توجد حظائر. يرجى إضافة حظيرة من تبويب "الحظائر" أولاً', 'warning');
                return;
            }
            
            pens.forEach(pen => {
                const available = pen.capacity - (pen.sheep_count || 0);
                ...
                const occupancyRate = pen.capacity > 0 ? Math.round(((pen.sheep_count || 0) / pen.capacity) * 100) : 0;
            ...
            console.log('✅ تم تحميل قائمة الحظائر بنجاح');
        } catch (error) {
            console.error('❌ خطأ في تحميل قائمة الحظائر:', error);
            UI.showAlert('خطأ في تحميل قائمة الحظائر: ' + error.message, 'error');
            
            // عرض رسالة في القائمة
            const penSelect = document.getElementById('pen');
            if (penSelect) {
                penSelect.innerHTML = `
                    <option value="">خطأ في تحميل الحظائر</option>
    ...
    async populateMotherOptions() {
        try {
            console.log('👩 جاري تحميل قائمة الأمهات...');
            ...
            if (motherSelect) {
                ...
                console.log(`✅ تم تحميل ${mothers.length} أم`);
            }
        } catch (error) {
